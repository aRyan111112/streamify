...
export async function login(req, res) {
    try {
        ...
        if(!user) { return res.status(401).json({message: "Invalid user email or password"}); }

        const isPasswordCorrect = await user.matchPassword(password);
        if(!isPasswordCorrect) { return res.status(401).json({message: "Invalid user email or password"}); }
...
export async function onBoarding(req, res) {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(userId, {
                ...req.body, // getting everything from body
                isOnboarded: true
        ...
        console.log("error in onBoarding method", error);
        res.status(500).json({message: "Internal server error"});
    }
}